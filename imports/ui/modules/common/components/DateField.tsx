...
interface Props {
  ...
  placeholder?: string;
  renderString?: (date?: Moment) => string;
  disabled?: boolean;
}
...
const DateField: React.FC<Props> = props => {
  ...
  const customString = React.useCallback(
    (dateInput?: Moment) => {
      return renderString
        ? renderString(dateInput)
        : `${dateInput ? dateInput.format(DATE_FORMAT) : intl.formatMessage({ id: 'chosenDate' })}`;
    },
    [intl, renderString],
  );
  ...
  const [dateFormatStr, setDateFormatStr] = React.useState(customString(date));
  ...
  const onBlur = React.useCallback(
    (e: React.FocusEvent<HTMLDivElement>) => {
      ...
      setFocus(false);
      onChange(time);
      setDateFormatStr(customString(time));
    },
    [onChange, time, customString, inputRef, parent],
  );
  ...
  React.useEffect(() => {
    setTime(date);
    setDateFormatStr(customString(date));
  }, [intl, date, customString]);
