...
export function RowCustom(props: {
  ...
  return (
    <>
      ...
      >
        {extendRow && (
          <TableCell style={{ width: 40 }} onClick={(e: any) => e.stopPropagation()}>
            <IconButton
              onClick={() => {
                setOpen(!open);
...
const TableCustom: React.FC<Props> = props => {
  ...
  return (
    <Paper id={id} className={`${classes.paper} ${className?.paper}`} style={style}>
      {header}
      <div className={`${classes.cover} ${className?.cover}`}>
        ...
        >
          <Table stickyHeader={stickyHeader} className={className?.table} style={styleTable}>
