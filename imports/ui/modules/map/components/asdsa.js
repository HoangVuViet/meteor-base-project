...
class LayerControl extends MapControl {
  ...
  addGroupedLayer = (layer, name, checked, group) => {
    ...
    this.setState((prevState, props) => {
      ...
      currentGroup = currentGroup
        ? [...currentGroup.filter((x) => x.name !== name), { layer, name, checked, group }]
  ...
  toggleLayer = (layerInput) => {
    ...
    layers[group] = layers[group].map((l) => {
      ...
      l.checked
        ? this.props.leaflet.map.addLayer(layer) && console.log(name, 'adding this layer')
    ...
    return (
      <React.Fragment>
        {ReactDOM.createPortal(
          <Paper onMouseEnter={this.openMenu} onMouseLeave={this.closeMenu} {...this.props}>
            {this.state.menuOpen && (
              <div style={{ padding: 10 }}>
                {Object.keys(this.state.layers).map((g) => {
                  return (
                    <React.Fragment key={g}>
                      <ListItem button onClick={() => this.onCollapseClick(`${g}`)}>
                      ...
                      <Typography />
                      <Divider />
                      <Collapse in={this.isMenuOpen(g)} timeout="auto" unmountOnExit>
                        <List>
